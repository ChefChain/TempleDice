...
async function initializeAgoraChat() {
    ...
    client.join(appID, channelName, token, uid).then((uid) => {
        ...
        // Initialize Agora Chat UIKit
        initializeAgoraChatUIKit(uid);
    }).catch((err) => {
...
function initializeAgoraChatUIKit(uid) {
    const appKey = "411225172#1429501";  // Your Agora Chat AppKey
    const containerId = "chatContainer";

    if (typeof AgoraChatUIKit === 'undefined') {
        alert("AgoraChatUIKit is not loaded. Please check the script tag for Agora Chat UIKit.");
        return;
    }
